...
func TestPartitionsWithMesh(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			serverHelmValues := map[string]string{
				"global.datacenter": "dc1",
				"global.image":      "ashwinvenkatesh/consul@sha256:822f8c33fe9c0fb42184f6ee719d55071d5aaf399df32af0e580c63d5272b893",
			...
			_, _, err = consulClient.ConfigEntries().Set(defaultPartitionExports, &api.WriteOptions{Partition: "default"})
			require.NoError(t, err)
			_, _, err = consulClient.ConfigEntries().Set(secondaryPartitionExports, &api.WriteOptions{Partition: "secondary"})
			require.NoError(t, err)
			...
			if c.secure {
				...
				logger.Log(t, "creating intention")
				intention.Sources[0].Partition = "secondary"
				_, _, err := consulClient.ConfigEntries().Set(intention, &api.WriteOptions{Partition: "default"})
				require.NoError(t, err)
				intention.Sources[0].Partition = "default"
				_, _, err = consulClient.ConfigEntries().Set(intention, &api.WriteOptions{Partition: "secondary"})
				require.NoError(t, err)
			}
